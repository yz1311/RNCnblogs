    ...
    static creatDateData(minDate,maxData,labelUnit)
    {
        ...
        for (let i = beginYear; i <= nowYear; i++) {
            ...
            for (let j = startJ; j < endJ; j++) {
                ...
                let day = [];
                if (j === 2) {
                    for (let k = startDateFlag ? beginDate : 1; k < (endDateFlag ? nowDate + 1 : 29); k++) {
                        day.push(k + DAYUNIT);
                    }
                    ...
                    for (let k = startDateFlag ? beginDate : 1; k < (endDateFlag ? nowDate + 1 : 32); k++) {
                        day.push(k + DAYUNIT);
                    }
                }
                else {
                    for (let k = startDateFlag ? beginDate : 1; k < (endDateFlag ? nowDate + 1 : 31); k++) {
                        day.push(k + DAYUNIT);
                    }
                }
                let _month = {};
