...
import {getLocalBlogListByPage, saveBlogListToLocal} from "../blog/blog_index_sagas";
...
export function* getPersonSignature(action) {
    yield* invokeCommonAPI({
        ...
        action: action,
        resolveResult: (result)=>{
            let matches = result.result.match(/class="headermaintitle"[\s\S]+?>[\s\S]+?<\/h2/);
            if(matches&&matches.length>0)
            {
...
export function* getUserAliasByUserName(action: SagaAction<getUserAliasByUserNameRequest>) {
    yield* invokeCommonAPI({
        ...
        resolveResult: (result)=>{
            ...
            if(matches&&matches.length>0)
            {
                for (let match of matches)
                {
                    try {
                        ...
                        let userName = temp[0].replace('title type="text">','').replace('</title','');
                        temp = match.match(/blogapp>[\s\S]+?<\/blogapp/);
                        let userAlias = temp[0].replace('blogapp>','').replace('</blogapp','');
                        ...
                        userList.push({
                            ...
                            iconUrl: userAvatar || 'https://pic.cnblogs.com/face/sample_face.gif',
            ...
            {
                let filter = userList.filter(x=>x.displayName === action.meta.parData.request.userName);
...
export function* watchProfileIndex() {
    yield all([
        takeOrCancel(actionTypes.PROFILE_GET_PERSON_INFO, actionTypes.PROFILE_CLEAR_PERSON_INFO, getPersonInfo),
        takeOrCancel(actionTypes.PROFILE_GET_PERSON_BLOG_LIST, actionTypes.PROFILE_CLEAR_PERSON_BLOG_LIST, getPersonBlogList),
        // takeOrCancel(actionTypes.PROFILE_GET_PERSON_SIGNATURE, '', getPersonSignature),
       takeOrCancel(actionTypes.PROFILE_GET_PERSON_ALIAS_BY_NAME, '', getUserAliasByUserName),
    ]);
}
