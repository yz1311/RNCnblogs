...
export function* setLoginCode(action) {
    ...
    // }));
    yield fork(userLogin,createSagaAction(actionTypes.LOGIN_USERLOGIN,{
        request: `client_id=${gBaseConfig.clientId}&client_secret=${gBaseConfig.clientSecret}&grant_type=authorization_code&code=${payload.code}&redirect_uri=https://oauth.cnblogs.com/auth/callback`,
...
export function* userLogin(action) {
    yield* invokeCommonAPI({
        ...
        successAction: function* (action, payload) {
            ...
            //     }
            // });
            const {meta:{parData:{callback}}} = action;
            callback&&callback();
            NavigationHelper.goBack()
            gStorage.save('loginInfo', payload);
            //获取用户信息
            yield fork(getUserInfo,createSagaAction(actionTypes.LOGIN_GET_USER_INFO,undefined));
        }
    });
}
...
export function* logout(action) {
    const {payload: {successAction}} = action;
    //清除本地存储
