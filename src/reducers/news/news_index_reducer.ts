...
export default handleActions(
  {
    ...
    [actionTypes.NEWS_GET_HOT_WREEK_LIST]: (state: State, action) => {
      ...
      if (!action.error) {
        state.hotWeekNewsList = state.hotWeekNewsList
          .slice(0, (pageIndex - 1) * pageSize)
          .concat(payload.result);
        state.hotWeekNewsList_noMore =
          (payload.result || []).length === 0 ||
          (payload.result || []).length < pageSize;
      }
      if (pageIndex > 1) {
      ...
      state.getHotWeekNewsListResult = actionToResult(
        action,
        null,
        state.hotWeekNewsList,
      );
    },
    ...
    [actionTypes.NEWS_GET_RECOMMENDED_NEWS_LIST]: (state: State, action) => {
      ...
      if (!action.error) {
        state.recommendedNewsList = state.recommendedNewsList
          .slice(0, (pageIndex - 1) * pageSize)
          .concat(payload.result);
        state.recommendedNewsList_noMore =
          (payload.result || []).length === 0 ||
          (payload.result || []).length < pageSize;
      }
      if (pageIndex > 1) {
      ...
      state.getRecommendedNewsListResult = actionToResult(
        action,
        null,
        state.recommendedNewsList,
      );
    },
    [actionTypes.NEWS_CLEAR_RECOMMENDED_NEWS_LIST]: (state: State, action) => {
      ...
      state.getRecommendedNewsListResult =
        initialState.getRecommendedNewsListResult;
      state.recommendedNewsList_noMore =
        initialState.recommendedNewsList_noMore;
    },
    ...
    [actionTypes.NEWS_GET_COMMENT_LIST]: (state: State, action) => {
      ...
      //防止出现数据重复的现象
      if (!action.error) {
        state.newsCommentList = state.newsCommentList
          .slice(0, (pageIndex - 1) * pageSize)
          .concat(payload.result);
        state.newsCommentList_noMore =
          (payload.result || []).length === 0 ||
          (payload.result || []).length < pageSize;
        state.curCommentPageIndex = pageIndex;
      }
      state.getNewsCommentListResult = actionToResult(
        action,
        null,
        state.newsCommentList,
      );
    },
    ...
    [actionTypes.HOME_REFRESH_DATA_TIME]: (state: State, action) => {
      ...
      if (state.selectedNews.postDateDesc) {
        state.selectedNews.postDateDesc = StringUtils.formatDate(
          state.selectedNews.DateAdded,
        );
      }
    },
  },
