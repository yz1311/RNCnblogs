...
export default handleActions( {
    [actionTypes.BOOKMARK_GET_LIST]:(state: State,action)=> {
        ...
        if(!action.error) {
            ...
            state.bookmarkList = state.bookmarkList.slice(0,(pageIndex-1)*pageSize).concat(payload.result);
            state.bookmarkList_noMore = (payload.result||[]).length === 0 || (payload.result||[]).length < pageSize;
        }
        state.getBookmarkListResult = actionToResult(action,null,state.bookmarkList);
    },
