...
import {actionToResult, handleActions, createReducerResult} from '../../utils/reduxUtils';
...
export default handleActions( {
    [actionTypes.KNOWLEDGEBASE_GET_IST]:(state: State,action)=> {
        ...
        if(!action.error) {
            ...
            state.kbList = state.kbList.slice(0,(pageIndex-1)*pageSize).concat(payload.result);
            state.kbList_noMore = (payload.result||[]).length === 0 || (payload.result||[]).length < pageSize;
        }
