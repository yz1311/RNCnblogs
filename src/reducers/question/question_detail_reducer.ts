...
export default handleActions( {
    ...
    [actionTypes.QUESTION_DELETE_ANSWER]:(state: State,action)=> {
        ...
        if(!action.error) {
            state.selectedQuestion.AnswerCount = state.selectedQuestion.AnswerCount + 1;
            //将问题的回答数量更改
    ...
    [actionTypes.QUESTION_CLEAR_ANSWER_COMMENT_LIST]:(state: State,action)=> {
        state.answerCommentList = initialState.answerCommentList;
        state.getAnswerCommentListResult = initialState.getAnswerCommentListResult;
    },
    [actionTypes.QUESTION_COMMENT_ANSWER]:(state: State,action)=> {
        const {meta: {parData: {request: {questionId, answerId, commentId}}}} = action;
        //将回答的评论数量更改
        for (let answer of state.answerList)
        {
    ...
    [actionTypes.QUESTION_MODIFY_QUESTION]:(state: State,action)=> {
        const {meta: {parData: {request: {questionId, Title, Content, Flags, UserID}}}} = action;
