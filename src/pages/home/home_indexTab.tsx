...
export default class home_indexTab extends YZTabItem<IProps,any>{
    ...
    render() {
        ...
        return (
            <View style={[{flexDirection:'row',backgroundColor:gColors.bgColorF,borderBottomWidth:gScreen.onePix,borderBottomColor:gColors.color999},this.props.containerStyle]}>
                ...
                {showSearchButton?
                    <TouchableOpacity
                        ...
                        style={{width:searchButtonWidth,justifyContent:'center',alignItems:'center'}}
                        onPress={()=>{
    ...
    renderTab=(data,index,isTabActive,onPressHandler)=>
    {
        ...
        return(
            <TouchableOpacity key={index} activeOpacity={activeOpacity} onPress={()=>this.props.goToPage(index)} style={[tabContentStyle]}>
                <View style={{flexDirection:'row',justifyContent:'center',alignItems:'center',paddingVertical:16}}>
                    <Text style={[{color:textColor,fontSize:gFont.size16},this.props.textStyle]}>{tabDatas[index]}</Text>
                </View>
            </TouchableOpacity>
        );
    }
