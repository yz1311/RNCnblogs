...
export default class question_item extends PureComponent<IProps,any> {
    ...
    _onDelete = ()=>{
        ...
        Alert.alert('','是否删除该提问?',[{
            ...
            onPress:()=>{
                ...
                deleteQuestionFn&&deleteQuestionFn({
                    request: {
                        questionId: item.Qid
                    },
                    successAction: ()=>{
                        //如果是在详情，则返回到列表界面
                        if(NavigationHelper.navRouters[NavigationHelper.navRouters.length-1].routeName === 'QuestionDetail')
                        {
    ...
    render() {
        ...
        //那个地址是错误的，无法访问
        if(item.QuestionUserInfo.Face && item.QuestionUserInfo.Face !== 'https://pic.cnblogs.com/avatar/sample_face.gif')
        {
                    ...
                    <View style={{backgroundColor:gColors.bgColorF,paddingVertical:10,paddingHorizontal:8}}>
                        <View style={{flexDirection:'row',alignItems:'center'}}>
                            <TouchableOpacity
                                ...
                                onPress={()=>{
                                    ServiceUtils.viewProfileDetail(this.props.dispatch,item.QuestionUserInfo.Alias,faceUrl);
                                }}
                        ...
                        >
                            {item.Award>0?
                            <MaterialCommunityIcons name="coin" size={20} color={gColors.colorff}/>:null}
                            {item.Award>0?
                            <Text style={{color:gColors.colorff}}>{item.Award+'   '}</Text>:null}
                        ...
                        <View style={{flexDirection:'row'}}>
                            {showAll ?
                                ...
                                :
                                <Text
                                    style={[{color: gColors.color333, fontSize: gFont.sizeDetail, marginVertical: 4},Styles.text4Pie]}
                                    {...detailProps}
                                    selectable
                                >
                                    {item.Summary}
                        ...
                        <View style={{flexDirection:'row',alignItems:'center',paddingBottom:8,flexWrap: 'wrap'}}>
                            {
                                tags.map((x,xIndex)=>{
                                    return (
                                        <Tag
...
const Tag = ({item, index, style})=>{
    ...
    return (
        <TouchableOpacity
            ...
            style={[{height:height,justifyContent:'center',borderRadius:6,paddingHorizontal:8,backgroundColor:gColors.borderColor,
        marginLeft:index!==0?8:0},style]}>
            <Text style={{color:gColors.themeColor,fontSize:gFont.size12}}>{item}</Text>
        </TouchableOpacity>
    );
}
