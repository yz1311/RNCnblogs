...
const StatusStarModal:FC<IProps> = ({isVisible,onVisibleChange, statusOtherInfo, loadOtherInfoResult, loadData})=>{
    return (
        <Modal
        ...
        >
            <View style={{width: Theme.deviceWidth*0.9, borderRadius: Theme.px2dp(10),
                ...
                <StateView
                    ...
                    >
                        {
                            (statusOtherInfo.今日闪存明星||[]).map((item, index)=>{
                                return (
                                    <TouchableOpacity
                                        onPress={()=>{
                                            // onVisibleChange&&onVisibleChange(false);
...
export default React.memo(StatusStarModal);
