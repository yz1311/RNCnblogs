...
export default class profile_person extends YZBaseDataPage<IProps,IState> {
    ...
    _renderHeader = ()=>{
        ...
        return (
            <YZStateView getResult={this.props.getPersonInfoResult}
                         placeholderTitle="暂无数据"
                         errorButtonAction={this.getPersonInfo}>
                <View style={{paddingTop:avatorRadius,borderRadius:10,marginTop:16}}>
                    <View style={{paddingHorizontal:8,paddingTop:avatorRadius,paddingBottom:15,backgroundColor:gColors.bgColorF}}>
                        <View>
                            <Text style={{fontSize:gFont.size24,fontWeight:'bold',marginTop:10}}>{personInfo.nickName}</Text>
                            <View
                                onLayout={({nativeEvent})=>{
                                    this.namePositionY = nativeEvent.layout.y + nativeEvent.layout.height;
                                }}
                    ...
                    <Image
                        ...
                        style={{width:avatorRadius*2,height:avatorRadius*2,borderRadius:avatorRadius,position:'absolute',
                            left:15,borderWidth:gScreen.onePix,borderColor:gColors.borderColor}}
                        source={{uri: avatorUrl}}/>
                    <View style={{height:15}}/>
                    <View style={{backgroundColor:gColors.themeColor,height:40,paddingLeft:10,justifyContent:'center'}}>
                        <Text style={{color:gColors.bgColorF,fontSize:gFont.size18,fontWeight:'bold'}}>博客</Text>
                    </View>
                </View>
            </YZStateView>
        );
    ...
    _handleScroll = (event)=>{
        ...
        let {title} = ((this.props.navigation.state || {params:undefined}).params || {title:undefined});
        title = title || '园友';
        ...
        else if(curScrollY<this.namePositionY&&title==personInfo.nickName)
        {
            this.props.navigation.setParams({
                title: '园友'
            });
                ...
                {this.props.loadDataResult.success && this.props.dataList.length == 0 ?
                    ...
                    <YZStateView getResult={this.props.loadDataResult}
                        ...
                        <YZFlatList
                            ...
                            ItemSeparatorComponent={() => <View style={{height: 0, backgroundColor: 'transparent'}}/>}
                        />
                    </YZStateView>
                }
            </View>
