...
// @ts-ignore
@YZBackHandler
export default class blog_detail extends YZBaseDataPage<IProps,any> {
    scrollPosition = 0;
    static propTypes = {
    ...
    static navigationOptions = ({navigation})=>{
        ...
        const {title, headerRight} = (state||{}).params || {title: undefined,headerRight: undefined};
        return {
            title: title || '博文',
    ...
    componentDidMount() {
        ...
        this.props.navigation.setParams({
            headerRight: <TouchableOpacity
                ...
                ref={ref=>this.fromView = ref} onPress={()=>{
                this.showMenu();
            }}>
    ...
    render() {
        ...
        {
            commentHtml = `<div style="margin-top: 30px;display: flex;flex-direction: column;align-items: center;color:${gColors.color666}">-- 暂无评论 --</div>`;
        }
        else
        {
            ...
            for (let comment of visibleCommentList)
            {
                commentHtml += `
                    <div style="display: flex; flex-direction: row;padding-top: 10px;">
                        ...
                        src="${comment.FaceUrl||'https://pic.cnblogs.com/avatar/simple_avatar.gif'}" />
                        <div style="display: flex; margin-left: 10px;flex-direction: column;flex: 1;">
                            <span style="font-weight: bold;">
                                <span style="color: salmon;">#${comment.Floor}楼</span>&nbsp;&nbsp;
                                <span>${comment.Author}</span>
                            </span>
                            <span style="font-size: 15px;color: #666666;margin-top: 8px;">${comment.Body}</span>
                            <span style="font-size: 15px;color: #999999;margin-top: 8px;">${StringUtils.formatDate(comment.DateAdded)}</span>
            ...
            if(showMoreButton)
            {
                commentHtml += `<div 
                                ...
                                <span style="color:${gColors.themeColor};font-size: medium;">点击查看全部评论</span>
                            </div>`;
            }
        }
        let html = `<html>
                ...
                <body style="padding: 0px;margin: 8px;"><div><div><h3>${item.Title}</h3>
                <span style="color:#666666;font-size: small">${item.Author}&nbsp;&nbsp;&nbsp;发布于&nbsp;${item.postDateDesc}</span>
                </div>${data.body}</div>
