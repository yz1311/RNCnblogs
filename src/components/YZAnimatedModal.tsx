...
export default class YZAnimatedModal extends Component<IProps,{}> {
    ...
    render() {
        ...
        if(this.state.keyboardHeight!=0)
        {
            ...
            if((gScreen.height-this.modalHeight)/2-20>this.state.keyboardHeight){
            ...
            {
                if(__IOS__) {
                    marginBottom = this.state.keyboardHeight - (gScreen.height - this.modalHeight) / 2 + 20;
                    //20是随便的一个值，因为实际上有点偏移
                    const maxMargin = gScreen.height - gScreen.statusBarHeight - this.modalHeight - 20;
        ...
        return (
            <Animated.View style={[styles.container, {opacity: opacityAnimatedValue}]}>
                <TouchableOpacity
                    activeOpacity={1}
                    style={styles.cover}
                    onPress={()=>{
                ...
                >
                    <Animated.View
                        ...
                        style={[styles.contentWrapper, {transform: [{scaleX: scaleAnimatedValue}, {scaleY: scaleAnimatedValue}]},{marginBottom:marginBottom},this.props.contentWrapperStyle]}
                    >
                        <TouchableOpacity
                            ref="modalContent"
                            activeOpacity={1}>
                            ...
                            {this.props.bottom?this.props.bottom:(<TouchableOpacity
                                activeOpacity={0.75}
                                style={[{
                                    height: 50,
                                ...
                                onPress={this._close}
                            >
                                <Text style={{fontSize: gFont.size16, color: gColors.themeColor}}>确定</Text>
                            </TouchableOpacity>)}
                            {this.props.overflow}
                        </TouchableOpacity>
                    </Animated.View>
