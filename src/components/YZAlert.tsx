...
export default class YZAlert extends Component<IProps,{}>
{
    ...
    {
        return(
            <View style={{borderTopLeftRadius:6,borderTopRightRadius:6,paddingTop:10}}/>
        );
    }
    ...
    renderContent()
    {
        ...
        return(
            <View>
                <View style={{paddingHorizontal:20,paddingBottom:10,alignItems:'center'}}>
                    {title?
                        <Text style={{fontSize:gFont.size16,color:gColors.color333,marginBottom:8}}>{title}</Text>:null}
                    <Text style={{fontSize:gFont.size15,color:gColors.color333}}>{message}</Text>
                </View>
            </View>
        )
    }
    ...
    renderBottom()
    {
        const {showCancelButton,showConfirmButton,
        ...
        if(showCancelButton&&showConfirmButton) {
            return (
                <View style={{flexDirection: 'row'}}>
                    ...
                    >
                        <Text style={{fontSize: gFont.size15, color: cancelButtonColor}}>{cancelText}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                        activeOpacity={0.75}
                        style={[{
                    ...
                    >
                        <Text style={{fontSize: gFont.size15, color: confirmButtonColor}}>{confirmText}</Text>
                    </TouchableOpacity>
                </View>
            );
        }
        else
        {
            return (
                <View style={{flexDirection: 'row'}}>
                    ...
                    >
                        <Text style={{fontSize: gFont.size15, color: gColors.color0}}>{cancelText}</Text>
                    </TouchableOpacity>
                </View>
            );
        }
