...
const RightItem = ({onPress, text, type, rightTandI, color}) => {
  ...
  return (
    <TouchableOpacity
      ...
      <Image
        source={rightTandI}
        style={{height: 15, width: 15, marginRight: gMargin / 2}}
      />
...
export default class YZHeader extends Component<IProps, {}> {
  ...
  render() {
    ...
    const backgroundStyle =
      type == 'default'
        ? {backgroundColor: '#fff'}
        : {backgroundColor: gColors.themeColor, borderBottomWidth: 0};
    ...
    return (
      <View
        ...
        <View
          ...
          {showGoBack && (
            <LeftItem onPress={backAction} type={type} leftTitle={leftTitle} />
          )}
          <Text style={[styles.title, fontStyle, titleStyle]}>
            {title || ''}
          </Text>
          {rightTitle && (
            <RightItem
              ...
              onPress={rightAction}
              type={type}
              rightTandI={rightTandI}
              color={this.props.rightTitleColor}
            />
          )}
          {rightIcon && (
            <RightIconItem icon={rightIcon} onPress={rightAction} />
          )}
          {renderLeftItem && (
            <TouchableOpacity
              activeOpacity={0.75}
...
const styles = StyleSheet.create({
  header: {
    ...
    height: isAndroid
      ? 50 + gScreen.statusBarHeight
      : 44 + gScreen.statusBarHeight,
    // width: screenW,
