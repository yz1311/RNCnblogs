...
export default class YZFlatList extends Component<IProps,IState>
{
    ...
    _renderFooter = () => {
        ...
        return (
            <View style={[{height: 40,flexDirection:'row',justifyContent: 'center', alignItems: 'center'},footerContainerStyle]}>
                {noMore?null:<ActivityIndicator color={gColors.themeColor} style={{marginRight:7}}/>}
                <Text style={[{textAlign:'center', color: gColors.color666, fontSize: gFont.size13},footerTextStyle]}>{promptTitle}</Text>
            </View>
        );
    }
